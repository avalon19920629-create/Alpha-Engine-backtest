...
def write_profile_outputs(out, returns, profiles, selected, alpha_turnover):
    out=Path(out); out.mkdir(parents=True,exist_ok=True)
    summary=pd.DataFrame({k:alpha.metrics(v) for k,v in returns.items()}).T; summary.index.name="Strategy"; summary["Turnover"]=0.0
...
def run_profiles(prices, us, jp, profiles, start, end, out):
    strategies,selected,turn=alpha.run_backtest(prices,us,jp,start,end); allr={"Alpha_Always_Only":strategies["Alpha_Always"]}
    ...
    return write_profile_outputs(out,pd.DataFrame(allr),profiles,selected,turn)
...
def main():
    ...
    summary,v=run_profiles(prices,us,jp,profiles,a.start,a.end,a.output_dir); print(summary[["CAGR","Max_Drawdown","Sharpe","Calmar"]]); print(v)
